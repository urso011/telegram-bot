...
async def options_gates(c: Client, m: CallbackQuery):
    ...
    orgn.append([InlineKeyboardButton(text="❮ ❮", callback_data="dobro")])
    kb = InlineKeyboardMarkup(inline_keyboard=orgn)

    await m.edit_message_text(
        "<b>🔃 Selecione quantos % de dobro saldo</b>", reply_markup=kb
    )
...
async def dobro(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="❮ ❮", callback_data="dobro")]
        ]
    )
    save()
    await m.edit_message_text(
        f"<b>✅ dobro de porcentagem alterado com sucesso. Porcentagem atual:→ {gate.title()}</b>", reply_markup=kb
    )
