...
@Client.on_callback_query(filters.regex(r"^comprar_cc mix$"))
    ...
    for level, price in levels_list:
        levels.append(
            InlineKeyboardButton(
                text=f"Mix {level} | R$ {price}",
                callback_data=f"comprar_cc mix {level}",
            )
        )
...
@Client.on_callback_query(filters.regex(r"^comprar_cc mix (?P<quantity>\d+)"))
@lock_user_buy
async def buy_mix(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="❮ ❮", callback_data="comprar_cc"),
            ],
        ]
    )
    ...
    if len(ccs_list) < quantity:
        return await m.answer(
            "⚠️ Não hã CCs disponiveis para o tamanho do mix requisitado.",
            show_alert=True,
        )
    ...
    if len(sold_list) < quantity:
        return await m.edit_message_text(
            "⚠️ Infelizmente não consegui completar sua requisisão de mix.",
            reply_markup=kb,
        )
    ...
    for new_cc in sold_list:
        ...
        list_dados = new_cc + (user_id, f"mix {quantity}", False)
    ...
    await m.message.reply_text(
        "✅ Compra realizada com sucesso. Clique no botÃ£o abaixo para voltar para o menu principal.",
        reply_markup=kb,
    )
